...
.e	tst.l	d0		;
	...
	eori.w	#8,-6(a1,d1.w)	; update mask colour
.next	swap	d6		;
	dbf	d6,.xoring	;
	dbf	d7,.loop	;
...
.loop1	move.w	(a3)+,d0	;
	...
	neg.w	d0		;
.a	lsl.w	#4,d0		;
	btst.b	#0,-10(a0,d0.w)	; test cutpoint
	dbne	d6,.loop1	;
	tst.w	d6		;
	bmi.b	.next		; no cut point
	beq.w	leave		; mistake
	subq.w	#1,d6		;
...
.b	lsl.w	#4,d0		;
	...
	tst.w	d6		;
	bmi.w	leave		; mistake
	move.w	-12(a0,d0.w),2(a1)

	eori.w	#8,10(a1)	; update mask colour index
	lea	16(a1),a1	;
