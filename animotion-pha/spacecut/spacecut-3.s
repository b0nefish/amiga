...
.e	tst.l	d0		;
	bmi.b	.next		;
	lsl.w	#4,d1		;
	eor.w	d6,-12(a1,d1.w)	; update vector colour index
.next	swap	d6		;
	dbf	d6,.xoring	;
	dbf	d7,.loop	;
...
.loop	movem.w	(a2),d1/d4	;
	...
	clr.l	6(a2)		; clear flags + cut point
	move.b	6(a4),d0	;
	move.b	6(a5),d1	;
	eor.b	d1,d0		;
...
.zero	movem.w	(a4),d1-d3	;
	...
	move.w	(a4),8(a2)	; set vector cut point index
	addq.w	#1,(a4)		; index++
...
cube_vectors
	...
	dc.w	2,3,0,0,0,0,0,0	; word 2 => color
